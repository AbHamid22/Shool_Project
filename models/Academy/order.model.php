...
class Order implements JsonSerializable{
	...
	public function set($id,$customer_id,$order_date,$delivery_date,$shipping_address,$order_total,$remark,$status_id,$discount,$vat,$created_at,$updated_at){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}orders(customer_id,order_date,delivery_date,shipping_address,order_total,remark,status_id)values('$this->customer_id','$this->order_date','$this->delivery_date','$this->shipping_address','$this->order_total','$this->remark','$this->status_id')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}orders set customer_id='$this->customer_id',order_date='$this->order_date',delivery_date='$this->delivery_date',shipping_address='$this->shipping_address',order_total='$this->order_total',paid_amount=',remark='$this->remark',status_id='$this->status_id',discount='$this->discount',vat='$this->vat',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
...
";
	...
	public static function total_sale($criteria=""){
		global $db,$tx;
		$result =$db->query("select sum(order_total) from {$tx}orders $criteria");
		list($count)=$result->fetch_row();
			return $count;
	}
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		...
		while($order=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= Event::button(["name"=>"show", "value"=>"Details", "class"=>"btn btn-info", "route"=>"order/show/$order->id"]);
			...
			$html.="<tr><td>$order->id</td><td>$order->customer</td><td>$odate</td><td>$ddate</td><td>$order->order_total</td></td> $action_buttons</tr>";
		}
		$html.="</table>";
