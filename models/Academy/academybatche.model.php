...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select b.id,b.name,c.name class from {$tx}academy_batches b,{$tx}academy_current_classes c where c.id=b.current_class_id $criteria limit $top,$perpage");
		$html="<table class='table'>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Current Class </th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Current Class </th></tr>";
		}
		while($academybatche=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$academybatche->id</td><td>$academybatche->name</td><td>$academybatche->class</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		global $db,$tx,$base_url;
			$result=$db->query("select b.id,b.name,c.name class from {$tx}academy_batches b,{$tx}academy_current_classes c where c.id=b.current_class_id and b.id={$id}");
		$academybatche=$result->fetch_object();
		$html="<table class='table'>";
		$html.="<tr><th colspan=\"2\">Batch Show</th></tr>";
		$html.="<tr><th>Id</th><td>$academybatche->id</td></tr>";
		$html.="<tr><th>Name</th><td>$academybatche->name</td></tr>";
		$html.="<tr><th>Current Class Id</th><td>$academybatche->class</td></tr>";
