...
class MoneyReceiptDetail implements JsonSerializable{
	...
	public static function Filter($mr_id){
		global $db,$tx;
		$result=$db->query("select m.id,m.money_receipt_id,p.name,m.price,m.qty from {$tx}money_receipt_details m,{$tx}products p where p.id=m.product_id and money_receipt_id='$mr_id'");
		$data=[];
		while($moneyreceiptdetail=$result->fetch_object()){
			$data[]=$moneyreceiptdetail;
		}
