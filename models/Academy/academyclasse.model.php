...
class AcademyClasse extends Model implements JsonSerializable{
	...
	// public static function filter2($id){
	// 	global $db,$tx;
	// 	//Update field name after where keyword if don't have name field
	// 	$result=$db->query("select r.id,c.name as class,s.name subject,t.name teacher,r.day,r.time from {$tx}academy_routines r,{$tx}academy_classes c,{$tx}academy_subjects s,{$tx}academy_teachers t where r.academy_class_id=c.id and r.academy_subject_id=s.id and r.academy_teacher_id=t.id and r.id='$id'");
...
";
	...
	static function html_row_details($id){
		...
		$academyclasse=$result->fetch_object();
		$html="<table class='table'>";
		$html.="<tr><th colspan=\"2\">Class Show</th></tr>";
