...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select r.id,s.name student,su.name subject,t.name term,e.name exam_type,r.mark,r.fullmark from {$tx}academy_results r,{$tx}academy_students s,{$tx}academy_subjects su,{$tx}academy_terms t,{$tx}academy_exam_types e where s.id=r.academy_student_id and
		su.id=r.academy_subject_id and t.id=r.academy_term_id and e.id=r.academy_exam_type_id  $criteria limit $top,$perpage");
		...
		while($academyresult=$result->fetch_object()){
			...
			$html.="<tr><td>$academyresult->id</td><td>$academyresult->student</td><td>$academyresult->subject</td><td>$academyresult->term</td><td>$academyresult->exam_type</td><td>$academyresult->mark</td><td>$academyresult->fullmark</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		global $db,$tx,$base_url;
	$result=$db->query("select r.id,s.name student,su.name subject,t.name term,e.name exam_type,r.mark,r.fullmark from {$tx}academy_results r,{$tx}academy_students s,{$tx}academy_subjects su,{$tx}academy_terms t,{$tx}academy_exam_types e where s.id=r.academy_student_id and
		su.id=r.academy_subject_id and t.id=r.academy_term_id and e.id=r.academy_exam_type_id and r.id={$id}");
		$academyresult=$result->fetch_object();
		$html="<table class='table'>";
		$html.="<tr><th colspan=\"2\">Result Show</th></tr>";
		$html.="<tr><th>Id</th><td>$academyresult->id</td></tr>";
		$html.="<tr><th>Academy Student Id</th><td>$academyresult->student</td></tr>";
		$html.="<tr><th>Academy Subject Id</th><td>$academyresult->subject</td></tr>";
		$html.="<tr><th>Academy Term Id</th><td>$academyresult->term</td></tr>";
		$html.="<tr><th>Academy Exam Type Id</th><td>$academyresult->exam_type</td></tr>";
	...
	static function html_student_reports($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select s.id,s.name,fathers_name,mothers_name,b.name batch,created_at,dob,photo,address from {$tx}academy_students s,{$tx}academy_batches b where b.id=s.academy_batch_id $criteria limit $top,$perpage");
		$html="<table class='table'>";
		if($action){
		...
		while($academystudent=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= Event::button(["name"=>"report", "value"=>"Reports", "class"=>"btn btn-info", "route"=>"academyresult/report/$academystudent->id"]);
	...
	// 	$html="<table class='table'>";
	// 	$html.="<tr><th>Subject</th><th>MCQ/<br>Obtain</th><th>Full marks</th></tr>";
	// 	$total=0;
	// 	$full_total=0;
	...
	public static function report($id, $term_id = 1) {
		...
		if (empty($results)) {
			return "<p style='font-weight: bold; color: red;'>No results found for this student.</p>";
		}
	
		// Inline CSS to mimic an Excel sheet
		$style = "
			<style>
				table {
					width: 100%;
		...
		foreach ($results as $result) {
			...
			$mark = intval($result->mark);
			$fullmark = intval($result->fullmark);
		...
		// Adding total row
		$html .= "<tr style='font-weight: bold; background-color: #d1e7fd;'>
					<td>Total</td>
					<td>$total</td>
					<td>$full_total</td>
				  </tr>";
